...
撮影時、Sky Logger App は次の基本6項目から 正式ハッシュ（SHA-256） を生成します：
...
※さらに、**ペア撮影モード**（空と地上の同時記録）が有効な場合は、上記の6項目に以下の2項目を加えた**最大8項目**でハッシュが生成されます：
...
📂 リポジトリの構成
パス / ページ	内容
verify.html	QRコードから開く、ログ照合用ページ
firestore_viewer.html	Firestore ↔ GitHub の整合性を一括検証するツール
hashes/	公開ハッシュファイル（ログ1件ごとに1ファイル）
...
（※ペア撮影モードのログを再計算する際は、pairGroupId と role も含める必要があります）
